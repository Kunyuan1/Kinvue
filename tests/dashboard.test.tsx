...
describe('what they said, on every card (KV-110)', () => {
  it('shows an answer whether or not a rule fired on it', async () => {
    // "Slept well" fires nothing; "slept badly" fires `poor-sleep`. Before
    ...
    const rows = await screen.findAllByText(/^They said:/, { exact: false })
    const said = rows.map((r) => r.parentElement?.textContent ?? '')
    expect(said).toContainEqual(expect.stringContaining('slept well'))
    expect(said).toContainEqual(expect.stringContaining('slept badly'))
    // The rule still says what counted, separately from what was said.
