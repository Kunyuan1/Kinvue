...
| Type | Notes |
|---|---|
| `Vitals` | `pulseRateBpm`, `breathingRateBrpm`, `hrvRmssdMs`, `hrvSdnnMs`, plus `confidence`, `stable` and `durationSec`. Any metric may be `null`. `confidence` describes the readings actually reported: per metric, the ones the SDK called settled, falling back to all of that metric’s readings when it settled on none. It is `null` when nothing rated the readings at all, and a null withholds the verdict rather than scoring it (KV-12). A pulse or breathing rate that reported no confidence of its own is `null` when another metric in the capture was rated, so it cannot ride on that metric's number (KV-79). |
